...
class Y21D08 : AocDays() {
    ...
    private fun analyze(line: String): Int {
        ...
        rosetta[0] = standard.filter { it.length == 6 }.filterNot { it == rosetta[6][0] }
            .filterNot { it.containsAll(rosetta[4][0]) }
        rosetta[9] = standard.filter { it.length == 6 }.filterNot { it == rosetta[6][0] }
            .filterNot { it == rosetta[0][0] }
        rosetta[3] = standard.filter { it.length == 5 }.filter { it.containsAll(rosetta[1][0]) }
        rosetta[5] = standard.filter { it.length == 5 }.filterNot { it == rosetta[3][0] }
            .filter { rosetta[6][0].containsAll(it) }
        rosetta[2] = standard.filter { it.length == 5 }.filterNot { it == rosetta[3][0] }
            .filterNot { it == rosetta[5][0] }
